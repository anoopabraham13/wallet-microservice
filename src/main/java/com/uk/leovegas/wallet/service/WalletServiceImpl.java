...
import com.uk.leovegas.wallet.exception.EntityNotFoundException;
...
import com.uk.leovegas.wallet.repository.TransactionRepository;
...
public class WalletServiceImpl implements WalletService {
    ...
    public Wallet createPlayerWallet(PlayerWalletRequest playerWalletRequest) {
        Player player = getPlayer(playerWalletRequest.getPlayerId());

        Wallet wallet = Wallet.builder().balance(0d).currencyCode(playerWalletRequest.getCurrencyCode()).build();
        player.setWallet(wallet);
        walletRepository.saveAndFlush(wallet);
        playerRepository.saveAndFlush(player);
        log.debug("The wallet having walletId: {}, is created for the playerId: {}", wallet.getId(), player.getId());
        return wallet;
    }
    ...
    public Wallet performCredit(Long playerId, Transaction transaction) {
        ...
        playerRepository.saveAndFlush(player);
        log.debug("The wallet having walletId: {}, is credited for the playerId: {}", playerWallet.getId(), player.getId());
        return playerWallet;
    }
    ...
    public Wallet performDebit(Long playerId, Transaction transaction) {
        ...
        player.getTransactions().add(transaction);
        playerRepository.saveAndFlush(player);
        log.debug("The wallet having walletId: {}, is debited for the playerId: {}", playerWallet.getId(), player.getId());
        return playerWallet;
    }
    ...
    private Player getPlayer(Long playerId) {
        Player player =playerService.getPlayer(playerId);
        if(null == player)
            throw new EntityNotFoundException("Player is not available for the playerId: "+ playerId);
    ...
    private Wallet getWallet(Long playerId, Player player) {
        ...
        if(null == playerWallet)
            throw new EntityNotFoundException("Player wallet is not available for the playerId: "+ playerId);
    ...
    private void validateTransaction(Transaction transaction, Player player, TransactionType transactionType) {
        ...
        if((transaction.getTransactionType() == TransactionType.DEBIT) && (player.getWallet().getBalance() < transaction.getAmount()))
            throw new InvalidTransactionException("Invalid transaction due to not enough balance in the wallet " +
                    "for the playerId: "+ player.getId());
    }
}
